...
SystemPromptsDialog::SystemPromptsDialog(QWidget* parent)
    : QDialog(parent), ui(new Ui::SystemPromptsDialog), selectedRow(-1) {
    ui->setupUi(this);

    connect(ui->addButton, &QPushButton::clicked, this,
            &SystemPromptsDialog::addPrompt);
    connect(ui->editButton, &QPushButton::clicked, this,
            &SystemPromptsDialog::editPrompt);
    connect(ui->deleteButton, &QPushButton::clicked, this,
            &SystemPromptsDialog::deletePrompt);
    connect(ui->moveUpButton, &QPushButton::clicked, this,
            &SystemPromptsDialog::movePromptUp);
    connect(ui->moveDownButton, &QPushButton::clicked, this,
            &SystemPromptsDialog::movePromptDown);
    connect(ui->resetButton, &QPushButton::clicked, this,
            &SystemPromptsDialog::resetToDefaults);

    connect(ui->buttonBox, &QDialogButtonBox::accepted, this,
            &SystemPromptsDialog::saveChanges);
    connect(ui->buttonBox, &QDialogButtonBox::rejected, this, &QDialog::reject);

    connect(ui->promptsList, &QListWidget::currentRowChanged, this,
            &SystemPromptsDialog::onPromptSelectionChanged);
    connect(ui->promptsList, &QListWidget::itemChanged, this,
            &SystemPromptsDialog::onPromptToggled);

    loadPrompts();
    updateButtonStates();
}

SystemPromptsDialog::~SystemPromptsDialog() { delete ui; }
...
void SystemPromptsDialog::addPrompt() {
    ...
    QLabel* tipLabel =
        new QLabel("Tip: Use clear, concise instructions for best AI results.");
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (!enabledCheck->isChecked()) {
            QList<SystemPrompt> all =
                SystemPrompts::instance()->getAllPrompts();
            if (!all.isEmpty()) {
                SystemPrompts::instance()->setPromptEnabled(all.last().id,
                                                            false);
            }
        }
...
void SystemPromptsDialog::editPrompt() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        SystemPrompts::instance()->updateCustomPrompt(selected.id, name,
                                                      prompt);
        SystemPrompts::instance()->setPromptEnabled(selected.id,
...
void SystemPromptsDialog::resetToDefaults() {
    QMessageBox::StandardButton reply =
        QMessageBox::question(this, "Reset Prompts",
                              "This will:\n"
...
void SystemPromptsDialog::onPromptToggled() {
    ...
    if (row >= 0 && row < currentPrompts.size()) {
        ...
        SystemPrompts::instance()->setPromptEnabled(currentPrompts[row].id,
                                                    enabled);
...
void SystemPromptsDialog::updateButtonStates() {
    ...
    ui->moveDownButton->setEnabled(hasSelection &&
                                   selectedRow < currentPrompts.size() - 1);
}

bool SystemPromptsDialog::validatePrompt(const QString& name,
    ...
    if (name.length() > 100) {
        QMessageBox::warning(this, "Invalid Input",
                             "Prompt name is too long (max 100 characters).");
        return false;
    }
