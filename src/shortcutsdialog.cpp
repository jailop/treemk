...
void ShortcutsDialog::setupUI()
{
    ...
    QLabel *titleLabel = new QLabel(tr("<h2>Keyboard Shortcuts Configuration</h2>"), this);
    layout->addWidget(titleLabel);
    
    QLabel *infoLabel = new QLabel(tr("Click on a shortcut to edit it. Press Enter to confirm or Esc to cancel."), this);
    ...
    shortcutsTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
    
    connect(shortcutsTable, &QTableWidget::cellDoubleClicked, this, &ShortcutsDialog::onCellClicked);
...
void ShortcutsDialog::loadShortcuts()
{
    ...
    addShortcut(tr("App Navigation"), tr("Follow Link (Mouse)"), tr("Ctrl+Click"));
    addShortcut(tr("App Navigation"), tr("Follow Link (Keyboard)"), tr("Ctrl+Enter"));
...
void ShortcutsDialog::onCellClicked(int row, int column)
{
    ...
    QLabel *label = new QLabel(tr("Press the key combination for: %1").arg(actionItem->text()), editDialog);
    layout->addWidget(label);
    
    QKeySequenceEdit *keyEdit = new QKeySequenceEdit(QKeySequence::fromString(currentShortcut), editDialog);
...
void ShortcutsDialog::onSave()
{
    ...
    for (int row = 0; row < shortcutsTable->rowCount(); ++row) {
        ...
        ShortcutManager::Action action = static_cast<ShortcutManager::Action>(actionData.toInt());
        QTableWidgetItem *shortcutItem = shortcutsTable->item(row, 2);
