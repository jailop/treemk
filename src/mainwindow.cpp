...
void MainWindow::save() {
  ...
  if (tab->filePath().isEmpty()) {
    ...
    treeView->notifyFileSaving(tab->filePath());
    if (tab->saveFile()) {
...
void MainWindow::saveAs() {
  ...
  treeView->notifyFileSaving(filePath);
  if (tab->saveFileAs(filePath)) {
    currentFilePath = filePath;
    linkParser->buildLinkIndex(currentFolder);
    statusBar()->showMessage(
        tr("File saved as: %1").arg(QFileInfo(filePath).fileName()), 3000);
  } else {
    QMessageBox::warning(this, tr("Error"), tr("Could not save file!"));
  }
}
...
void MainWindow::onWikiLinkClicked(const QString &linkTarget) {
  ...
  QString targetPath =
      linkParser->resolveLinkTarget(linkTarget, currentTab->filePath());

  if (targetPath.isEmpty()) {
    QMessageBox::StandardButton reply = QMessageBox::question(
        this, tr("Link Not Found"),
        tr("The file '%1' does not exist in the current directory or its "
...
void MainWindow::onFileModifiedExternally(const QString &filePath) {
  ...
  statusBar()->showMessage(
      tr("File modified externally: %1").arg(fileInfo.fileName()), 5000);
  QMessageBox::StandardButton reply = QMessageBox::question(
...
void MainWindow::findAndReplace() {
  ...
  connect(replaceButton, &QPushButton::clicked,
          [tab, findEdit, replaceEdit, this]() {
            QTextCursor cursor = tab->editor()->textCursor();
            if (cursor.hasSelection() &&
                cursor.selectedText() == findEdit->text()) {
  ...
  connect(replaceAllButton, &QPushButton::clicked,
          [tab, findEdit, replaceEdit, &dialog, this]() {
            QString searchText = findEdit->text();
            ...
            QMessageBox::information(
                &dialog, tr("Replace All"),
                tr("Replaced %1 occurrence(s)").arg(count));

            statusBar()->showMessage(tr("Replaced %1 occurrence(s)").arg(count),
                                     3000);
          });
...
void MainWindow::insertImage() {
  ...
  QString altText = QInputDialog::getText(
      ...
      tr("Enter alt text for the image (optional):"), QLineEdit::Normal,
...
void MainWindow::attachDocument() {
  ...
  QString documentPath = QFileDialog::getOpenFileName(
      ...
      currentFolder.isEmpty() ? QDir::homePath() : currentFolder,
      tr("All Files (*);;PDF Files (*.pdf);;Word Documents (*.doc "
         "*.docx);;Text Files (*.txt);;Spreadsheets (*.xls *.xlsx "
         "*.ods);;Presentations (*.ppt *.pptx *.odp);;Audio Files (*.mp3 *.wav "
         "*.ogg *.m4a *.flac *.aac);;Video Files (*.mp4 *.webm *.ogv *.avi "
         "*.mov *.mkv)"));
  ...
  if (!attachmentsDir.exists()) {
    if (!currentDir.mkdir(attachmentsDirName)) {
      QMessageBox::warning(this, tr("Error"),
                           tr("Failed to create attachments directory '%1'.")
                               .arg(attachmentsDirName));
      return;
    }
  }
  ...
  if (QFile::exists(destPath)) {
    QMessageBox::StandardButton reply = QMessageBox::question(
        this, tr("File Exists"),
        tr("File '%1' already exists in the attachments folder. Overwrite?")
            .arg(destFileName),
        QMessageBox::Yes | QMessageBox::No);
  ...
  bool ok;
  QString linkText =
      QInputDialog::getText(this, tr("Document Link Text"),
                            tr("Enter link text for the document (optional):"),
                            QLineEdit::Normal, docInfo.baseName(), &ok);
...
void MainWindow::insertWikiLink() {
  ...
  QString targetPath = QFileDialog::getOpenFileName(
      this, tr("Select Target File"), currentDir.path(),
      tr("Markdown Files (*.md *.markdown);;All Files (*)"));
  ...
  QString displayText = QInputDialog::getText(
      this, tr("Wiki Link Display Text"),
  ...
  statusBar()->showMessage(tr("Wiki link inserted (relative to current file)"),
                           3000);
}
...
void MainWindow::insertHeader() {
  ...
  QString item =
      QInputDialog::getItem(this, tr("Insert Header"),
...
void MainWindow::insertCodeBlock() {
  ...
  bool ok;
  QString language = QInputDialog::getText(this, tr("Code Block"),
...
void MainWindow::insertLink() {
  ...
  bool ok;
  QString text =
      QInputDialog::getText(this, tr("Insert Link"), tr("Enter link text:"),
                            QLineEdit::Normal, "", &ok);
...
void MainWindow::insertTable() {
  ...
  int cols = QInputDialog::getInt(this, tr("Insert Table"),
                                  tr("Number of columns:"), 3, 2, 10, 1, &ok);
...
void MainWindow::quickOpen() {
  ...
  connect(&dialog, &QuickOpenDialog::fileSelected, this,
...
void MainWindow::onTabCloseRequested(int index) {
  ...
  if (shouldSave) {
    ...
    QMessageBox::StandardButton reply = QMessageBox::question(
        this, tr("Unsaved Changes"),
        tr("'%1' has unsaved changes. Do you want to save before closing?")
            .arg(tab->fileName()),
        QMessageBox::Save | QMessageBox::Discard | QMessageBox::Cancel);
    ...
    if (reply == QMessageBox::Save) {
      if (tab->filePath().isEmpty()) {
        QString filePath = QFileDialog::getSaveFileName(
            this, tr("Save File"), currentFolder,
            tr("Markdown Files (*.md *.markdown);;All Files (*)"));
...
void MainWindow::exportToHtml() {
  ...
  QProcess process;
  QStringList args;
  args << currentFilePath << "-o" << outputPath << "--standalone"
       << "--mathjax"
...
void MainWindow::exportToPdf() {
  ...
  QProcess process;
  QStringList args;
  args << currentFilePath << "-o" << outputPath << "--pdf-engine=pdflatex"
       << "--metadata" << "title=" + QFileInfo(currentFilePath).baseName();
  ...
  if (process.exitCode() == 0) {
    ...
    QMessageBox::critical(this, tr("Export Failed"),
                          tr("Failed to export document:\n%1\n\nNote: PDF "
                             "export requires LaTeX to be installed.")
                              .arg(errorMsg));
  }
}
...
void MainWindow::exportToDocx() {
  ...
  QProcess process;
  QStringList args;
  args << currentFilePath << "-o" << outputPath << "--metadata"
       << "title=" + QFileInfo(currentFilePath).baseName();
...
void MainWindow::exportToPlainText() {
  ...
  QProcess process;
  QStringList args;
  args << currentFilePath << "-o" << outputPath << "--to" << "plain";
