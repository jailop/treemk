...
HelpDialog::HelpDialog(QWidget *parent)
  ...
  // Connect signals
  connect(ui->homeButton, &QPushButton::clicked, this, &HelpDialog::onHomeClicked);
  connect(ui->backButton, &QPushButton::clicked, this, &HelpDialog::onBackClicked);
  connect(ui->forwardButton, &QPushButton::clicked, this, &HelpDialog::onForwardClicked);
  connect(ui->tableOfContents, &QListWidget::currentRowChanged,
          this, &HelpDialog::onTableOfContentsItemClicked);
  connect(ui->searchLineEdit, &QLineEdit::textChanged,
          this, &HelpDialog::onSearchTextChanged);
  connect(ui->searchResultsList, &QListWidget::itemClicked,
          this, &HelpDialog::onSearchResultClicked);
  connect(ui->printButton, &QPushButton::clicked,
          this, &HelpDialog::onPrintClicked);
...
void HelpDialog::initializeTopics() {
  ...
  topics = {
    ...
    {"getting-started", tr("Getting Started"), tr("Quick introduction to TreeMk")},
    {"editor", tr("Editor Features"), tr("Editing capabilities and shortcuts")},
    {"navigation", tr("Navigation"), tr("Wiki links, outline, and search")},
    {"preview", tr("Preview Features"), tr("Rendering, formulas, and themes")},
    {"formulas", tr("Math Formulas"), tr("LaTeX mathematical notation")},
    {"code-blocks", tr("Code Blocks"), tr("Syntax highlighting for code")},
    {"diagrams", tr("Mermaid Diagrams"), tr("Creating diagrams with Mermaid")},
    {"ai-assistant", tr("AI Assistant"), tr("AI-powered writing help")},
    {"keyboard-shortcuts", tr("Keyboard Shortcuts"), tr("Complete shortcuts reference")}
  };
}
...
void HelpDialog::setupContent() {
  ...
  connect(contentView, &MarkdownPreview::wikiLinkClicked,
          this, &HelpDialog::onLinkClicked);
  connect(contentView, &MarkdownPreview::markdownLinkClicked,
          this, &HelpDialog::onLinkClicked);
...
void HelpDialog::loadHelpContent(const QString &fileName) {
  ...
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    QString errorMsg = tr("Could not load help topic: %1\n\nResource path: %2")
...
void HelpDialog::performSearch(const QString &searchTerm) {
  ...
  QList<SearchEngine::SearchResult> results = searchEngine->searchInFiles(filePaths, term);
  ...
  if (results.isEmpty()) {
    ...
    for (const SearchEngine::SearchResult &result : results) {
      ...
      QString displayText = QString("%1\n%2")
                              .arg(result.title.isEmpty() ? result.fileName : result.title)
                              .arg(result.context);
      
      QListWidgetItem *item = new QListWidgetItem(displayText);
      item->setData(Qt::UserRole, result.filePath);
      item->setToolTip(QString("Line %1: %2").arg(result.lineNumber).arg(result.context));
      ui->searchResultsList->addItem(item);
    }
    
    // Update results label
    ui->searchResultsLabel->setText(tr("Search Results (%1)").arg(results.size()));
  }
