...
QuickOpenDialog::QuickOpenDialog(const QString &path, const QStringList &recent, QWidget *parent)
...
void QuickOpenDialog::setupUI()
{
    ...
    connect(searchEdit, &QLineEdit::textChanged, this, &QuickOpenDialog::updateFileList);
    connect(fileListWidget, &QListWidget::itemDoubleClicked, this, &QuickOpenDialog::onItemDoubleClicked);
...
void QuickOpenDialog::updateFileList()
{
    ...
    if (pattern.isEmpty()) {
        for (const QString &recent : recentFiles) {
            ...
            QListWidgetItem *item = new QListWidgetItem(info.fileName() + " [Recent]");
...
QString QuickOpenDialog::getSelectedFile() const
{
    return selectedFile;
}
...
bool QuickOpenDialog::fuzzyMatch(const QString &pattern, const QString &text) const
{
    int patternIndex = 0;
    int textIndex = 0;
