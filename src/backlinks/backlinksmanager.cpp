...
BacklinksManager::BacklinksManager(QObject *parent)
    : QObject(parent) {
}

void BacklinksManager::buildBacklinks(const QMap<QString, QVector<QString>> &forwardLinks) {
  ...
  for (auto it = forwardLinks.constBegin(); it != forwardLinks.constEnd(); ++it) {
    ...
    for (const QString &linkTarget : linkTargets) {
      ...
      for (const QString &candidateNorm : normalizedCandidates) {
        for (auto fit = forwardLinks.constBegin(); fit != forwardLinks.constEnd(); ++fit) {
          const QString &scannedFile = fit.key();
          QString scannedNorm = normalizePath(scannedFile);
          
          if (candidateNorm.compare(scannedNorm, Qt::CaseInsensitive) == 0) {
...
QVector<QString> BacklinksManager::getBacklinks(const QString &filePath) const {
  ...
  for (auto it = backLinksMap.constBegin(); it != backLinksMap.constEnd(); ++it) {
