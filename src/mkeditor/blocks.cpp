...
void MarkdownEditor::swapTextBlocks(QTextBlock& block1, QTextBlock& block2) {
    ...
    int firstEnd =
        firstStart + firstBlock.length() - 1;  // -1 to exclude newline
    int secondStart = secondBlock.position();
    int secondEnd =
        secondStart + secondBlock.length() - 1;  // -1 to exclude newline

    QTextCursor tempCursor(document());
    tempCursor.beginEditBlock();

    // Replace second block's text (do this first so first block position stays
    // valid)
    ...
    int newPos =
        targetBlock.position() + qMin(relativePos, targetBlock.text().length());
