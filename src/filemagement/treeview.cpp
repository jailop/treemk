...
void FileSystemTreeView::setupView() {
    ...
    // Drag and drop enabled to support:
    // 1. Dragging files from file explorer to editor (creates wiki-links)
    // 2. Dragging files within file explorer (move/copy file operations)
    setDragEnabled(true);
    setAcceptDrops(true);
    setDropIndicatorShown(true);
    setDragDropMode(QAbstractItemView::DragDrop);
    // InternalMove means drag-drop within the tree view will move files
    // But dragging to external widgets (like the editor) still works
...
void FileSystemTreeView::onDirectoryChanged(const QString& path) {
    ...
    // QFileSystemModel automatically handles directory changes and updates the
    // view. No additional action needed here.
}
...
void FileSystemTreeView::contextMenuEvent(QContextMenuEvent* event) {
    ...
    if (hasSelection) {
        ...
        if (fileInfo.isFile()) {
            ...
            isMarkdownFile =
                (suffix == "md" || suffix == "markdown" || suffix == "txt");
            isNonMarkdownFile = !isMarkdownFile;
        }
    }
...
void FileSystemTreeView::addDirectoriesToWatcher(const QString& path) {
    ...
    if (path.contains("/systemd-private-") || path.startsWith("/proc") ||
        path.startsWith("/sys") || path.startsWith("/dev")) {
        return;
    }
...
void FileSystemTreeView::openFileWith() {
    QModelIndex index = currentIndex();
    if (!index.isValid()) {
        return;
    }
...
void FileSystemTreeView::onItemRenamed(const QModelIndex& topLeft,
    ...
    if (!renameOldPath.isEmpty()) {
        ...
        if (newPath != renameOldPath && QFile::exists(newPath) &&
            !QFile::exists(renameOldPath)) {
