...
void OpenAIProvider::setEndpoint(const QString &url) {
  endpoint = url;
}
...
void OpenAIProvider::setTimeout(int seconds) {
...
QString OpenAIProvider::buildSystemPrompt() const {
  return 
    ...
    "Do not include explanations or commentary unless specifically requested.";
}

void OpenAIProvider::process(const QString &prompt,
                              const QString &content,
                              std::function<void(const QString&)> onSuccess,
                              std::function<void(const QString&)> onError) {
  if (apiKey.isEmpty()) {
    onError("API key not configured. Please set your OpenAI API key in settings.");
    return;
  }
  ...
  netReq.setRawHeader("Authorization", QString("Bearer %1").arg(apiKey).toUtf8());
  
  QNetworkReply *reply = networkManager->post(netReq, 
  ...
  connect(reply, &QNetworkReply::finished, this, [=]() {
    ...
    if (reply->error() == QNetworkReply::NoError) {
      ...
      if (!errorDoc.isNull()) {
        QString apiError = errorDoc.object()["error"].toObject()["message"].toString();
        if (!apiError.isEmpty()) {
          errorMsg = apiError;
        }
      }
