...
void SystemPrompts::initializeDefaults() {
  ...
  p3.prompt = "Make the following text more concise while keeping key information:";
  p3.enabled = true;
  p3.isCustom = false;
  p3.order = order++;
  prompts.append(p3);
  ...
  p5.prompt = "Simplify the language in the following text for easier understanding:";
  p5.enabled = true;
  p5.isCustom = false;
  p5.order = order++;
  prompts.append(p5);
  ...
  p10.prompt = "Add relevant examples to illustrate the points in the following text:";
  p10.enabled = true;
  p10.isCustom = false;
  p10.order = order++;
  prompts.append(p10);
...
QList<SystemPrompt> SystemPrompts::getAllPrompts() const {
  return prompts;
}
...
void SystemPrompts::addCustomPrompt(const QString &name, const QString &prompt) {
  SystemPrompt p;
  p.id = generateId();
  p.name = name;
  p.prompt = prompt;
...
void SystemPrompts::updateCustomPrompt(const QString &id, const QString &name, const QString &prompt) {
  for (SystemPrompt &p : prompts) {
