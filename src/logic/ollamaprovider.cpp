...
void OllamaProvider::setEndpoint(const QString &url) {
  endpoint = url;
}
...
void OllamaProvider::process(const QString &prompt,
  ...
  connect(timer, &QTimer::timeout, this, [reply, onError]() {
    reply->abort();
    onError("Request timed out. The model might be too slow or unavailable.");
  });
  ...
  connect(reply, &QNetworkReply::finished, this, [=]() {
    ...
    if (reply->error() == QNetworkReply::NoError) {
      ...
      QJsonDocument resultDoc = QJsonDocument::fromJson(rawResponse.toUtf8());
      if (!resultDoc.isNull() && resultDoc.isObject()) {
        ...
        if (!result.isEmpty()) {
          ...
          onError("AI response did not contain expected 'result' field");
        }
      } else {
